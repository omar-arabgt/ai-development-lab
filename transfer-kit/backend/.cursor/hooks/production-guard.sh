...
if echo "$cmd" | grep -qiE 'DATABASE_URL=|--settings[= ][^ ]*(prod|staging)|(psql|pg_dump|pg_restore|mysql|mysqldump) .*(prod|staging|arabgt\.com)|manage\.py (shell|dbshell|flush)|git push .*(--force|-f( |$))'; then
  cat <<JSON
