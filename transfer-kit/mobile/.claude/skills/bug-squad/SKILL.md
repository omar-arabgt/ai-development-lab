...
6. Each agent, inside its own worktree:
   ...
   d. Fix the root cause (Flutter code only, minimal diff, match-the-file style).
   e. Run `flutter analyze` then `flutter test` (the FULL suite, not only the new test). All green or keep working — never ship red.
