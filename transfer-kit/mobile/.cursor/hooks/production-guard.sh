...
if echo "$cmd" | grep -qiE 'main_prod|environments/prod|fastlane .*(release|deploy|beta)|xcrun altool|upload.*(appstore|play)|flutter build (appbundle|ipa)|git push .*(--force|-f( |$))'; then
  cat <<JSON
